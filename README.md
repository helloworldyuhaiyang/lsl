...
Cue 是嵌在脚本正文里的表达提示，统一格式为 `[...]`，用来描述一句话该怎么说。

以 cue 为核心的口语复盘与脚本练习系统，面向「已有口语录音」和「可生成的对话脚本」两类输入。

## 项目目标

- 高质量口语复盘：转写、纠错、优化、可反复收听
- Cue 驱动脚本学习：每一句脚本都允许内嵌 `[...]` cue，并贯穿编辑、高亮、TTS
- Web First：先做 Web 上传和任务流转，后续再扩展 Desktop
- 可演进架构：存储、ASR、LLM、TTS 都可替换

## 产品定位（Phase 1）

LSL 是一个「以 cue 为核心的口语复盘与脚本练习工具」。

输入：
- 上传语音文件（mp3 / wav / m4a）
- 创建文本 Session（自己写脚本 / AI 生成带 cue 的对话脚本）
...
LSL 里的脚本不是纯文本，而是“脚本正文 + 内嵌 cue”组成的单一可编辑字符串。

- cue 格式统一使用方括号：`[语气平淡地回应]`
- cue 是脚本的一部分，不拆成独立字段
- cue 在前端编辑态中需要高亮显示，与 `revise-page` 当前橙色高亮体验保持一致
- TTS 会从完整脚本里提取 cue，再剥离出纯朗读文本

示例：

```text
[语气平淡地回应] Interesting, innit?
[随意附和] Yeah, right.
```
