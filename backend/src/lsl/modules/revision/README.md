...
- RevisionService 通过 `SessionService` 和 `TranscriptService` 获取 `session/transcript`，不跨模块直接访问别人的 repo。
- 当前阶段每个 `session` 只保留一份当前 revise 结果，不保存历史版本。
- `POST /revisions` 是异步生成接口：先创建 `revision_generation` job 并返回 `generating`。
- 用户编辑态只保存一个字段 `draft_text`，不再单独维护 `draft_cue`。

## 当前接口设计

- `POST /revisions` 创建或覆盖当前 session 的 revise job，立即返回当前 revision
- `GET /revisions?session_id={session_id}` 查询当前 session 的 revise 结果
- `GET /revisions/preview?session_id={session_id}` 查询当前 session 的章节规划和已生成 revise items，供前端轮询预览
...
- `REVISION_PROVIDER=llm` 时，调用大模型执行“先规划章节，再按 section 并发流式 revise”。
...
- 重新生成时会清空当前 revision items 上已有的 `draft_text` 和旧的 `plan_sections_json`。
- 通用 job runner 继续执行 LLM 任务；前端应轮询 `GET /revisions/preview`。
...
### 3) `GET /revisions/preview?session_id={session_id}`

返回当前 session 对应的 revision preview。

preview 包含两类数据：

- `revision.plan_sections`: 本次 revise 的章节规划，包括 `section_index / title / summary / start_seq / end_seq / target_utterance_count`
- `items`: 当前已生成并写库的 revise items

该接口用于前端轮询展示生成进度；不需要额外 preview 表，已生成建议仍来自 `revision_items`。

### 4) `PATCH /revisions/items/{item_id}`
...
1. 读取整段 transcript。
2. transcript 不超过 10 条 utterance 时直接作为一个 section；超过 10 条才调用 LLM 生成章节规划和 section summary。
3. 每个 section 带少量前后文并发调用 LLM 执行 revise；section 请求按 2 秒间隔依次提交，降低平台并发限流风险。
...
新库会按当前“单脚本字段 + span item + cue_language + plan_sections_json”结构创建 revision 表。

`revision_revisions.plan_sections_json` 保存本次生成的章节规划 JSON 数组，用于 preview 进度和后续 section summary 展示；已生成建议仍保存在 `revision_items`。
